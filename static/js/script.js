...
async function fetchAndRenderCustomIndexData(options = { force: false }) {
    ...
    try {
        ...
        if (!csvText || csvText.trim().length < 5) throw new Error("Received empty or invalid data from sheet.");
        ...
        customIndexError.textContent = `Error: ${err.message}`;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    customIndexCopyButton = document.getElementById('custom-index-copy-button'); customIndexExportButton = document.getElementById('custom-index-export-button');
    
    // --- Populate Input Array for Event Listeners ---
    ...
    allCalculatorInputs.forEach(input => {
        if (input) {
            input.addEventListener('input', () => { sanitizeNumericInput(input); updateAllCalculationsAndSave(); });
            input.addEventListener('blur', () => { sanitizeNumericInput(input); if (input.value.endsWith('.')) { input.value = input.value.slice(0, -1); } updateAllCalculationsAndSave(); });
        }
    });
    ...
    updateAllCalculationsAndSave(); // This performs the initial calculation on page load
