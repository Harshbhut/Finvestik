...
def download_and_process_band_data():
    """
    Downloads the NSE security list CSV using a robust, two-step session
    with different headers to mimic a real browser and avoid being blocked.
    """
    # --- Configuration ---
    BASE_URL = "https://nsearchives.nseindia.com/content/equities/sec_list_{date}.csv"
    MAX_RETRIES = 5
    # The URL to visit to get the initial, essential cookies
    ...
    # --- Header Sets (inspired by your reference code) ---
    # Header for the initial cookie-grabbing request
    cookie_header = {
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/129.0.0.0 Safari/537.36"
    }
    
    # A much more comprehensive header for the actual data download request
    data_header = {
    ...
    with requests.Session() as s:
        # Step 1: "Warm up" the session by visiting the homepage with a simple header
        try:
            print("Visiting NSE homepage to establish a session and get cookies...")
            s.get(NSE_HOME_URL, headers=cookie_header, timeout=20)
        ...
        for i in range(MAX_RETRIES):
            ...
            try:
                ...
                # The session 's' now has the cookies. We make the request with the powerful data_header.
    ...
    # --- The rest of your processing logic is unchanged and should work perfectly ---
    if csv_content:
        try:
