...
CONFIG = {
    ...
    "output_file": os.path.join(STATIC_DATA_DIR, "Chartink.json"),
    "output_version_file": os.path.join(STATIC_DATA_DIR, "version1.json"),
...
# --- 3. DATA MAPPING AND CALCULATION FUNCTIONS (LOGIC UNCHANGED) ---
...
def calculate_turnover_sma20(stocks: List[Dict], historical_data: List[Dict], trade_date: str):
    ...
    for stock in stocks:
        ...
        stock["TurnoverSMA20"] = round(sum(turnover_values) / len(turnover_values), 2) if turnover_values else 0
...
def prepare_and_save_data(stocks: List[Dict], timestamp: str):
    ...
    logging.info(f"Successfully saved {len(records)} stocks to: {CONFIG['output_file']}")
...
def main():
    ...
    logging.info(f"✅ Version file created at {CONFIG['output_version_file']} with timestamp {current_timestamp_ms}")
